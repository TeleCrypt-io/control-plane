...
func TestShutdownRegistrationAfterFailurePreservesListenerFailure(t *testing.T) {
	listenerErr := errors.New("listener failed")
	if err := shutdownRegistrationAfterFailure(&http.Server{}, listenerErr); !errors.Is(err, listenerErr) {
		t.Fatalf("shutdownRegistrationAfterFailure error = %v, want listener failure", err)
	}
}
