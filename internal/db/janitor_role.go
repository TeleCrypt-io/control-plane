...
func ValidateJanitorDatabaseContract(ctx context.Context, pool *pgxpool.Pool) error {
	...
	err := pool.QueryRow(ctx, `SELECT COALESCE(CASE
	 ...
	 WHEN NOT EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace JOIN pg_roles r ON r.oid=c.relowner WHERE n.nspname='cashier' AND c.relname='janitor_subscription_snapshot' AND c.relkind='v' AND r.rolname=$1) THEN 'janitor_subscription_snapshot view is missing or has wrong owner'
	 ...
	 WHEN NOT has_table_privilege(current_user, 'cashier.janitor_subscription_snapshot', 'SELECT') THEN 'janitor_subscription_snapshot is not readable'
...
func validateJanitorCashierViewColumns(ctx context.Context, pool *pgxpool.Pool) error {
	checks := []struct {
		name    string
		columns []string
	}{{"janitor_deployment_identity", []string{"server_name", "billing_environment"}}, {"janitor_lifecycle_actions", []string{"mxid", "revision", "action", "due_at", "desired_user_type"}}, {"janitor_subscription_snapshot", []string{"subscription_id", "status", "team_id", "provider_product_id"}}}
